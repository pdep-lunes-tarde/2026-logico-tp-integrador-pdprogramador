...
% correcion 3a sacamos el año del functor (hazaña).
% conmemoracion(Pueblo, Hazaña, AnioInicio).
...
% correcion 3b reemplazo del "is max()"
...
puebloMasLector(Pueblo, AnioDado) :-
    ...
    forall(cantidadDeHojasLeidas(_, AnioDado, OtrasHojas), MaxHojas >= OtrasHojas). % cambio not por un forall

puebloMusical(Pueblo, AnioDado) :-
    pueblo(Pueblo), % generador de pueblos
    findall(Hazania, recuerdaHazaniaPueblo(Pueblo, Hazania, AnioDado), ListaHazanias),
    list_to_set(ListaHazanias, HazaniasRecordadas), % list_to_set evita repetidos
    findall(HazaniasMusicales, (member(HazaniasMusicales, HazaniasRecordadas), recordadaPorCancion(Pueblo, AnioDado, HazaniasMusicales)), HazaniasMusicales),
    length(HazaniasRecordadas, CantidadHazaniasRecordadas), % musicales + no musicales = total de hazanias recordadas
    length(HazaniasMusicales, CantidadHazaniasMusicales),
    CantidadHazaniasMusicales > CantidadHazaniasRecordadas / 2.

recordadaPorCancion(Pueblo, AnioDado, Hazania) :- % auxiliar para verificar que si la persona recuerda la hazaña por cancion, esa cancion no este vencida.
    habitante(Persona, _, _, Pueblo),
    conocio(Persona, Hazania, cancion, AnioConocio),
    estaViva(Persona, AnioDado),
    recuerdaHasta(cancion, AnioDado, AnioConocio).
...
cadenaDeInspiracion(Inicial, Cadena) :-
    heroe(Inicial),
    inspiro(Inicial, Siguiente), 
    armarCadena(Siguiente, [Inicial], CaminoInvertido),
    reverse(CaminoInvertido, Cadena).

armarCadena(Actual, Visitados, [Actual | Visitados]).
armarCadena(Actual, Visitados, CadenaFinal) :-
    inspiro(Actual, Siguiente),
    not(member(Siguiente, Visitados)),
    armarCadena(Siguiente, [Actual | Visitados], CadenaFinal).
...
:- begin_tests(tpIntegrador, []).
    ...
    test(cadena_de_inspiracion_valida_con_multiples_eslabones) :- cadenaDeInspiracion(himmel, [himmel, frieren, fern, denken]).
    test(cadena_de_inspiracion_invalida_si_no_hay_relacion_de_inspiracion) :- not(cadenaDeInspiracion(denken, [denken, frieren])).
    test(cadena_de_inspiracion_no_permite_ciclos_repetidos_del_mismo_heroe) :- not(cadenaDeInspiracion(frieren, [frieren, fern, frieren])).
